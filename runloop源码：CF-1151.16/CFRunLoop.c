...
struct __CFRunLoop {
    ...
    __CFPort _wakeUpPort;			// used for CFRunLoopWakeUp 手动唤醒runloop的
    Boolean _unused;
    volatile _per_run_data *_perRunData;              // reset for runs of the run。 volatile:不稳定的 loop的端口。初始化runloop时设置，仅用于CFRunLoopWakeUp，CFRunLoopWakeUp函数会向_wakeUpPort发送一条消息
...
struct __CFRunLoopTimer {
    ...
    CFIndex _order;			/* immutable */
    CFRunLoopTimerCallBack _callout;	/* immutable */        //设置回调函数，回调指针
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    if (pthread_equal(t, kNilPthreadT)) {     //线程t为空则默认返回主线程runloop
    ...
    if (!__CFRunLoops) {
        ...
        //if的括号里的函数是比较第一个和第三个参数是否相等 如果相等 则把第二个参数原子性的赋值给内存位置
        if (!OSAtomicCompareAndSwapPtrBarrier(NULL, dict, (void * volatile *)&__CFRunLoops)) {
            CFRelease(dict);
        }
        CFRelease(mainLoop);
        //线程互斥锁pthread_mutex_lock(LP)
        __CFLock(&loopsLock);
    }
